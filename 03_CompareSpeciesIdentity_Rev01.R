...
summary(gam(prop.reads.common ~ log10(median.seqs+1):ReferDB, data = data, family = betar(link = 'logit')))
summary(gam(prop.reads.common ~ Year:ReferDB, data = data, family = betar(link = 'logit')))
summary(gam(prop.spes.new ~ log10(median.seqs+1):ReferDB, data = data, family = betar(link = 'logit')))
summary(gam(prop.spes.new ~ Year:ReferDB, data = data, family = betar(link = 'logit')))

mod1 <- gam(prop.reads.common ~ log10(median.seqs+1) + Year + ReferDB + SR_Re, data = data, family = betar(link = 'logit'))
mod2 <- gam(prop.spes.new ~ log10(median.seqs+1) + Year + ReferDB + SR_Re, data = data, family = betar(link = 'logit'))
...
betadisper_res_1 <- betadisper(dist_matrix, group = data_info$Continent)
betadisper_res_2 <- betadisper(dist_matrix, group = data_info$type)
betadisper_anova_1 <- anova(betadisper_res_1)
betadisper_anova_2 <- anova(betadisper_res_2)


db_rda_res_1 <- capscale(dist_matrix ~ Continent, data = data_info)
db_rda_res_2 <- capscale(dist_matrix ~ type, data = data_info)


sink('Res_Compare_SpeciesIdentity_nmds_sig.txt')
...
print(anosim_res_2)
print('-------------------------------')
print('betadisper for continents...')
print(betadisper_anova_1)
print('betadisper for type...')
print(betadisper_anova_2)
print('-------------------------------')
print('db-RDA for continents...')
anova(db_rda_res_1)
print('db-RDA for type...')
anova(db_rda_res_2)
sink()