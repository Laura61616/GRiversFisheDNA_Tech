...
#############################################################
####compare corrected SR with original ones--all datasets--GLMM
#############################################################
library(glmmTMB)
library(DHARMa)
library(MuMIn)
...
dt <- melt(data[,c(12,6,11,22,18:21)], id.vars = c('ID','Continent','ReferDB','SR'))
tapply(dt$value, dt$variable, function(x) var(x, na.rm=TRUE) / mean(x, na.rm=TRUE))
...
for (i in unique(dt$variable)) {
  ...
  # ---------- All Datasets ----------
  b <- glmmTMB(value ~ SR + (1|ID),
               data = dt_use,
               family = nbinom2(link="log"))
  
  # DHARMa
  r <- simulateResiduals(b, n = 1000)
  dis <- testDispersion(r)$p.value
  zin <- testZeroInflation(r)$p.value
  
  # R²
  r2_full <- r.squaredGLMM(b)[1, c("R2m","R2c")]
  marg_r2 <- unname(r2_full["R2m"])
  cond_r2 <- unname(r2_full["R2c"])
  
  fit <- summary(b)
  add1 <- data.frame(
    scale = i,
    type = 'All Datasets',
    Estimate = fit$coefficients$cond["SR","Estimate"],
    STD = fit$coefficients$cond["SR","Std. Error"],
    z = fit$coefficients$cond["SR","z value"],
    p = fit$coefficients$cond["SR","Pr(>|z|)"],
    cond_R2 = cond_r2,
    marg_R2 = marg_r2,
    Disp_p = dis,
    ZeroInf_p = zin
  ...
  # ---------- Global_db ----------
  dt_sub <- subset(dt_use, ReferDB == 'Global_db')
  b <- glmmTMB(value ~ SR + (1|ID),
               data = dt_sub,
               family = nbinom2(link="log"))
  r <- simulateResiduals(b, n = 1000)
  ...
  # ---------- Custom_db ----------
  dt_sub <- subset(dt_use, ReferDB == 'Custom_db')
  b <- glmmTMB(value ~ SR + (1|ID),
               data = dt_sub,
               family = nbinom2(link="log"))
  r <- simulateResiduals(b, n = 1000)
...
colnames(res) <- c('Scale','Type','Estimate', 'STD', 'z','p','cond_R2','marg_R2','Disp_p','ZeroInf_p')
write.table(res, 'Res_Compare_SpeciesRichness_GLMM.csv', row.names = F, sep = ',', quote = F)