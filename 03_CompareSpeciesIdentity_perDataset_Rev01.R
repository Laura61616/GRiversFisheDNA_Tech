...
sink('3_compare_Species_Identity_boxplot_perDataset_glmm2.log')
mod1 <- glmmTMB(prop.n.seqs ~ type2 + Year + ReferDB + (1|ID), data, weights = tot.n.seqs, family = binomial(link = 'logit'))
mod2 <- glmmTMB(prop.mean.seqs ~ type2 + Year + ReferDB + (1|ID), data, weights = tot.n.seqs, family = binomial(link = 'logit'))
summary(mod1)
summary(mod2)
...
y <- data$prop.n.seqs
N <- length(y)
data$prop.n.seqs_beta <- (y * (N - 1) + 0.5) / N
range(data$prop.n.seqs_beta)

y <- data$prop.mean.seqs
N <- length(y)
data$prop.mean.seqs_beta <- (y * (N - 1) + 0.5) / N
range(data$prop.mean.seqs_beta)

sink('3_compare_Species_Identity_boxplot_perDataset_glmm3.log')
mod1 <- glmmTMB(prop.n.seqs_beta ~ type2 + Year + ReferDB + (1|ID), data, family = beta_family(link = 'logit'))
mod2 <- glmmTMB(prop.mean.seqs_beta ~ type2 + Year + ReferDB + (1|ID), data, family = beta_family(link = 'logit'))
summary(mod1)
summary(mod2)
